...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    ...
    for i in range(num_gt):
        ...
        if (gt_name == current_cls_name):
            ...
            valid_class = 0
        elif current_cls_name == "Car".lower() and "Truck".lower() == gt_name:
            valid_class = 0
        else:
            valid_class = -1
        ignore = False
...
@numba.jit(nopython=True)
def compute_statistics_jit(overlaps,  # (预测框个数, 真值框个数)
                           gt_datas,
                           dt_datas,
                           ignored_gt,
                           ignored_det,
...
def eval_class(gt_annos,
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            ...
            print(
                f"#metric{metric} current_class {current_class} total_num_valid_gt: {total_num_valid_gt} in {difficulty}")
